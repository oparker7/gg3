...
# Likelihood and forward-backward algorithm for HMMs

def make_ll(spikes: np.ndarray, rates: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    """Broadcast-safe Poisson log-likelihood matrix ``ll[t, s]``.

    Parameters
    ----------
    spikes : (T,) array
        Observed spike counts for *one* trial.
    rates  : (K,) array
        Expected spike counts (rate·dt) for each discrete HMM state.
    eps    : float
        Lower bound to avoid ``log(0)`` when a state has rate 0.
    """
    rates = np.clip(rates, eps, None)
    return poisson_logpdf(spikes, rates)  # (T, K)


def run_fba(ll: np.ndarray, Tmat: np.ndarray, pi0: np.ndarray, *, filtering: bool = False) -> np.ndarray:
    """Posterior P[s_t | n₁:ᵀ] via `inference.hmm_expected_states`."""

    post, _ = hmm_expected_states(pi0, Tmat, ll, filter=filtering)
    return post  # shape (T, K)
...
def jump_time_error(p_up: np.ndarray, tau_true: int, thresh: float = 0.5):
    # Absolute error between true and inferred jump time
...
def run_one_setting(simulator,
    ...
    for j in range(N_trials):
        ...
        err = metric_fun(latent[j], est) if metric_fun is mae_ramp else metric_fun(est, latent[j])[0]
        errs.append(err)

        if j < 3:  # keep a few illustrative trials
            examples.append((latent[j], est, spikes[j]))
...
def sweep_grid(grid: list[tuple],
               ...
               *,
               K: int = 100,
               **kwargs):
    """Average error over a 2-D parameter sweep."""
...
def plot_example_ramp(x_true, x_hat, spikes, ax=None):
    ...
    ax.vlines(np.where(spikes > 0)[0], *ax.get_ylim(), color="grey", alpha=.2)
...
def plot_example_step(p_up, tau_true, tau_hat=None, ax=None):
    ...
    ax.axvline(tau_true, color="k", ls="--", alpha=.5, label=r"$\tau_{true}$")
...
# -----------------------------------------------------------------------------
#   Factory builders (simulator  +  HMM)
# -----------------------------------------------------------------------------
...
def build_ramp(beta: float,
    ...
    pi0  = np.zeros(K)
    pi0[0] = 1.0  # start at lowest x

    xgrid      = hmm.x_values
    rate_grid  = Rh * xgrid  # Poisson mean counts per bin
...
def build_step(m: int,
               ...
               *,
               ...
               x0: float = 0.2,
               Rh: float = 50.0):
    """Return (simulator, T, pi0, up-index/indices, rate-grid)."""
    ...
    if exact:
        up_idx    = np.array([K - 1])          # absorbing state
