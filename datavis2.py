    ...
    for i, b in enumerate(beta_list):
        for j, s in enumerate(sigma_list):
            ...
            for shape in shapes:
                ...
                bound_events_list.append(bound_events)

            # Flatten data and prepare color/offsets
            all_events = []
            all_bounds = []
            lineoffsets = []
            shape_colors = ['r', 'g', 'b']
            ...
            for k, (events, bounds) in enumerate(zip(events_list, bound_events_list)):
                for t in range(N_show):
                    ...
                    all_bounds.append(bounds[t])
                    lineoffsets.append(t + k * (N_show + 1))  # offset each shape block

            # Repeat colors for N_show trials per shape
            spike_colors = np.repeat(shape_colors[:len(shapes)], N_show)
            bound_colors_full = np.repeat(bound_colors[:len(shapes)], N_show)
